...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 **Tip**: Adjust your profile and click 'Regenerate Plan' to get different advice!")

# --- Main Content ---
st.title("🩺 Health Action Squad")
st.markdown(
    """
...
uploaded_files = st.file_uploader(
    "📄 Upload Health Report (Max 4 files)", 
...
if uploaded_files:
    if len(uploaded_files) > 4:
        st.warning("⚠️ Please upload a maximum of 4 files.")
    else:
        ...
        with col1:
            if st.button("🚀 Analyze Report", type="primary"):
                ...
                status_container = st.status("🕵️‍♂️ Starting analysis...", expanded=True)
                ...
                for update in call_upload_api_stream(uploaded_files, profile_data):
                    if update["type"] == "progress":
                        status_container.write(f"👉 {update['message']}")
                        status_container.update(label=f"🕵️‍♂️ {update['message']}")
